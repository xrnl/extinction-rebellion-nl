...
<div class="rwb">
  <div class="bg-blue px-3 py-lg-5 pb-5 text-center text-white rwb-cover-image" style="background: linear-gradient(rgba(0, 0, 0, 0.65), rgba(0, 0, 0, 0.45)), url('<?php the_field('rwb_cover_image_url'); ?>') no-repeat;">
    <div class="py-5">
      ...
      <?php if(get_field('rwb_caption')): ?>
        <h1 class="font-xr text-uppercase">
          <?php the_field('rwb_caption'); ?>
        </h1>
      <?php endif; ?>
    </div>

    <?php if(get_field('rwb_date')): ?>
      <h1 class="font-xr text-uppercase"><?php the_field('rwb_date'); ?></h1>
    <?php endif; ?>
    <?php if(get_field('rwb_location')): ?>
      <div class="rwb-location">
        <?php the_field('rwb_location'); ?>
      </div>
    <?php endif; ?>

    <?php if(get_field('rwb_cta_title')): ?>
      <div class="my-3">
        <a class="btn btn-lg btn-pink" href="#join">
          <?php the_field('rwb_cta_title'); ?>
        </a>
      </div>
    <?php endif; ?>

    <?php if(get_field('rwb_facebook_event_url')): ?>
      <a href="<?php the_field('rwb_facebook_event_url'); ?>" target="_blank" class="text-white text-reset text-underline">Facebook</a>
      &nbsp;
    <?php endif; ?>
    <?php if(get_field('rwb_facebook_event_url') && get_field('rwb_meetup_event_url')): ?>
      <?php _e('or', 'theme-xrnl') ?>
    <?php endif; ?>
    <?php if(get_field('rwb_meetup_event_url')): ?>
      &nbsp;
      <a class="text-white text-reset text-underline" href="<?php the_field('rwb_meetup_event_url'); ?>" target="_blank">Meetup</a>
    <?php endif; ?>
  </div>
  ...
  <?php if(get_field('rwb_join_title') && get_field('rwb_join_description') && get_field('sign_up_form_code')): ?>
  <div class="bg-pink py-sm-5 py-4">
    <a name="join"></a>
    <div class="container">
      <div class="row py-5 text-center">
        <div class="col-12 col-lg-8 mx-auto">
          <h2><?php the_field('rwb_join_title'); ?></h2>
  ...
  <?php if(get_field('rwb_why_protest') && get_field('rwb_demands')): ?>
  <div class="container text-center py-sm-5 py-4">
    <div class="row py-5">
      <?php if(get_field('rwb_why_protest')): ?>
        <div class="col-12 col-lg-8 mx-auto">
          <h2>
  ...
  <?php if(get_field('rwb_who_is_xr')): ?>
